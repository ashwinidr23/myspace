                   ...
                   for kver in kerconfig:
                        ...
                        for kitem in kver:
                         ...
                         if (kernelver == "" ):
                          ...
                          #empty_kernel_ver_host = [vm]
                          empty_kernel_ver_host_list.append(vm)
                         ...
                         if (kver[kitem].get("Version") == kernelver):
                            ...
                            #mailto = "Pramod.Raju@shell.com,Nishit.J.Patel@shell.com,John.Thiesfeld@shell.com,ashwini.dr@shell.com,grace.stellara@shell.com,sarang.bhat@shell.com"
...
if len(consolidatedvmlist) != 0:
    ...
    #mail_cc_list = "Ricardo.Gonzalez@shell.com,Nishit.J.Patel@shell.com,John.Thiesfeld@shell.com,ashwini.dr@shell.com,Pramod.Raju@shell.com,S.Patchamatla@shell.com,grace.stellara@shell.com,sarang.bhat@shell.com"
...
#send the empty kernel versions list to OS team
print("This is the list of hosts that has empty kernel versions")
print(empty_kernel_ver_host_list)
if len(empty_kernel_ver_host_list) != 0:
    fields = ['Hostname']
    cur_date = datetime.datetime.now()
    formatted_date = cur_date.strftime("%Y%m%d")
    filename = "emptykernelverhostlist_" + formatted_date + ".csv"
    dir_path = "/var/log/infra-security-patching-logs"
    if not os.path.exists(dir_path):
      os.makedirs(dir_path)
      print(f"Directory '{dir_path}' created successfully.")
    empty_kernel_ver_host_list_file_path = os.path.join(dir_path, filename)
    with open(empty_kernel_ver_host_list_file_path, "w") as file:
     csv_writer = csv.writer(file)
     csv_writer.writerow(fields)
     csv_writer.writerows(empty_kernel_ver_host_list)
    #mail_to_list4 = "GXITSOSOMPTHPCOS@shell.com"
    mail_to_list4 = "ashwini.dr@shell.com"
    mail_body4 = "Hi Team,\n PFA, The list of infra servers that have emtpy kernel versions. These hosts needs to be investigated and update with kernel version fact.\n"
    mail_command4 = f"echo -e '{mail_body4}' | mail -a {empty_kernel_ver_host_list_file_path} -s 'Infra Servers having empty kernel versions - consolidated list' {mail_to_list4}"
    subprocess.getoutput(mail_command4)
else:
    print("There are no servers with empty kernel versions")
