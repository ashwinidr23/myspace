...
kernelver = " "
kernel_str = subprocess.check_output("hammer fact list --search 'name=uname::release' ", shell=True)
kernelversions = kernel_str.decode().splitlines()
consolidatedvmlist = []
unsupportedvmlist = []
...
#        kernel_list = kernels.decode().splitlines()
        ...
        if vmlist == ['']:
         ...
         for vm in vmlist:
                  ...
                  if vmname in region:
                   #print(vm)
                   for k in kernelversions:
                    if vmname in k: 
                     keritem = k.split("|")
                     kernelver = (keritem[2]).strip()
                     ...
                     for kver in kerconfig:
                        ...
                        for kitem in kver:
                         ...
                         if (kernelver == "" ):
                          ...
                          #subprocess.getoutput(mail_command)
                          break
                          #print("keritem " + keritem + " vm " + vm + " kernel version " + kernelver )
                         ...
                         if (kver[kitem].get("Version") == kernelver):
                             print("kernerl version matched" + kver[kitem].get("Version"))
